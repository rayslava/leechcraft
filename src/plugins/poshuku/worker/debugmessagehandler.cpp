						...
						void Write (QtMsgType type, const char *message, bool bt)
						{
							...
							switch (type)
							{
								case QtDebugMsg:
									...
									break;
								case QtWarningMsg:
									name += "warning_lpw.log";
									break;
								case QtCriticalMsg:
									name += "critical_lpw.log";
									break;
								case QtFatalMsg:
									name += "fatal_lpw.log";
									break;
							...
							std::ofstream ostr;
							G_DbgMutex.lock ();
							ostr.open (QDir::toNativeSeparators (name)
									.toStdString ().c_str (), std::ios::app);
							ostr << "["
								<< QDateTime::currentDateTime ()
									.toString ("dd.MM.yyyy HH:mm:ss.zzz").toStdString ()
								<< "] ["
