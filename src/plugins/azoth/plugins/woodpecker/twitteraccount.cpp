	...
	void TwitterAccount::ChangeState (const EntryStatus& status)
	{
		...
		if (State_.State_ == SOnline)
		{
			...
			for (const auto& item : Proto_->GetParentProtocolPlugin ()->GetUserManager ()-> GetContainer ())
			{
				...
				auto *msg = new TwitterMessage(IMessage::DIn, IMessage::MTChatMessage, TE);
				msg->SetRichBody("<div id=\"message\"><b>Bold message: </b> message <img style='float:left;margin-right:4px' width='32px' height='32px' src='" + user->GetAvatarUrl () +  "'></div>");
